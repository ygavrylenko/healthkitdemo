...
/**
 ...
 @param articleIdOrName Article ID or URL Name to search for.
 @param error           Optional pointer to an error variable.
 
 @return `SCSArticle` matching the given ID, or `nil` if no article is
         found in the cache with that name.
 @see `SCSArticle`
 */
- (nullable SCSArticle*)articleWithId:(NSString*)articleIdOrName error:(NSError * _Nullable * _Nullable)error;
